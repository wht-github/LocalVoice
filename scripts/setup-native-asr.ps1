# Install the native Windows SenseVoice CPU environment used by the desktop client.
# Creates %LOCALAPPDATA%\LocalVoice\venv with CPU-only torch plus the ASR server stack.
param(
...
# Locate a system Python 3.10-3.13 (versioned names cover uv-managed installs).
$base = $null
foreach ($candidate in @('py', 'python', 'python3.13', 'python3.12', 'python3.11', 'python3.10')) {
    try {
        $version = & $candidate -c 'import sys; print(f"{sys.version_info.major}.{sys.version_info.minor}")' 2>$null
        if ($LASTEXITCODE -eq 0 -and $version -match '^3\.1[0-3]$') { $base = $candidate; break }
    } catch { }
}
if (-not $base) { throw '未找到 Python 3.10-3.13，请先安装 Python（勾选 Add to PATH）。' }
...
Write-Host '首次启动识别时 SenseVoiceSmall 模型会经 HF 镜像自动下载到用户缓存目录。'
