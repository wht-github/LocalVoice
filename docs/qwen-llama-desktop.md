...
安装命令见 [install.md](../install.md)。`setup-llama.ps1` 下载原始官方 ZIP、校验官方 release 给出的 SHA256，将程序与 CUDA DLL 放在同一目录；不配置全局 PATH、不安装 CUDA Toolkit、不调用 MSVC/NVCC。包内程序显示 `0.4.0-dev (build 10809, commit 5266f24da)`，这是官方包自身的版本字符串。
...
Slint 启动 `.venv/Scripts/python.exe` 运行 `asr_server.py`。SenseVoice 在此进程加载 CPU 模型；Qwen 通过 `llama_asr.py` 启动 `.runtime/llama/llama-server.exe`，Python 不加载 Qwen 或 CUDA PyTorch。
...
```powershell
$asrPython = Join-Path $PWD '.venv/Scripts/python.exe'
...
同日 v0.2.0 仓库内安装验收：使用已有 uv 0.11.0 / Python 3.12.12 新建 `.venv`，安装 torch / torchaudio 2.11.0+cpu，依赖检查、24 项 Rust 测试和 release 构建通过。四次模型切换在新环境中再次通过，报告为 `outputs/asr-models-local-check.json`。SenseVoice 的配套文件通过下载准备，936 MB `model.pt` 下载停滞后复用了原缓存中的同 revision 权重，SHA256 为 `833ca2dcfdf8ec91bd4f31cfac36d6124e0c459074d5e909aec9cabe6204a3ea`。这验证了本地部署和推理，未完成 SenseVoice 大权重从零下载的网络验收。
...
- 新机器首次 SenseVoice 加载较慢：等待模型下载和预热；后续复用 `.runtime/cache/huggingface` 缓存。
